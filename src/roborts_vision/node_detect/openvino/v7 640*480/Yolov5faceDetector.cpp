...
void YoloDetector::detet2origin(const Rect& dete_rect,Rect &out_rect,const vector<float> &points,vector<float> &out_points,float rate_to){
    //detect坐标转换到内部纯图坐标

    int ox = round(float(dete_rect.x)/rate_to);
    int oy = round(float(dete_rect.y)/rate_to);
    int ow = round(float(dete_rect.width)/rate_to);
    int oh =  round(float(dete_rect.height)/rate_to);
    out_rect = Rect (ox,oy,ow,oh);
    for (int i = 0; i < points.size(); i=i+2) {
        float  x=points[i]/rate_to;
        float  y=points[i+1]/rate_to;
...
void YoloDetector::to_final_objects(vector<Object> & final_objects,vector<int> & final_id, vector<Rect> & origin_rect,vector<float> & origin_rect_cof,vector<vector<float> > &origin_point){
    for (int i = 0; i < final_id.size(); ++i) {
        ...
        detet2origin(resize_rect, rawrect, points, output_points, r);
