...
bool HikvisionBasicCam::InitHandle(uint32_t n_device_index) {
  ...
  n_ret_ = MV_CC_EnumDevices(
      MV_USB_DEVICE,
      &m_stDevList);  // 枚举所有的相机，并且返回错误码（如果错误的话）
  if (MV_OK != n_ret_) {
  ...
  memcpy(&m_stDevInfo, m_stDevList.pDeviceInfo[n_device_index],
  ...
  // TODO:啥东西来的
  // //这个变量是为了保存打开的相机的序号，以备重启相机时使用,函数传入的参数是由yaml导入的，默认是0,并且2021赛季的车一直是一个视觉用的摄像头
  n_device_index_ = n_device_index;
  guid_ =
      std::string((char *)m_stDevInfo.SpecialInfo.stUsb3VInfo
                      .chDeviceGUID);  // 获取设备的guid号,是个唯一标注的序号
  return true;
}

bool HikvisionBasicCam::InitHandle(
    std::string cam_guid) {  // 这个函数好像是要根据给定的guid号来链接设备
  MV_CC_DEVICE_INFO_LIST m_stDevList = {0};
  ...
  for (int i = 0; i < m_stDevList.nDeviceNum; i++) {
    ...
    if (cam_guid ==
        std::string(
            (char *)m_stDevInfo.SpecialInfo.stUsb3VInfo.chDeviceGUID)) {  // ？
      n_ret_ = MV_CC_CreateHandle(&handle_, &m_stDevInfo);
      if (MV_OK != n_ret_) {
...
bool HikvisionBasicCam::OpenCamera() {
  ...
  // 连接设备
  n_ret_ = MV_CC_OpenDevice(handle_, nAccessMode,
...
bool HikvisionBasicCam::sendTrigger() {
...
// false
bool HikvisionBasicCam::GetMat_Triger(sensor_msgs::msg::Image &image_msg) {
  /* 旧取帧法 */
...
OPEN_LOOP:
  ...
  if (n_ret_ != MV_OK) {  // 如果取一帧超过时间，或者取帧错误，都不对
    ...
    if (RestartTimes < 5) {
      TDT_ERROR("the %d times !   Get Frame Error , will restart the camera \n",
                RestartTimes);
      RestartCamera();
      sendTrigger();
      ++RestartTimes;
  ...
  // int tdt_mode = 0 ;
  if (tdt_mode == 0)
    LoadParam::ReadParam("camera", "armor_exposure", expose_time_);
  else if (tdt_mode == 1 || tdt_mode == 2)
    LoadParam::ReadParam("camera", "buff_exposure", expose_time_);
  else {
  ...
  // unsigned char *p_image = (unsigned char *)malloc(st_img_info_.nWidth *
  // st_img_info_.nHeight * 4 + 2048); if (p_image == NULL) {
  //     return false;
  // }
...
bool HikvisionBasicCam::GetMat(sensor_msgs::msg::Image &image_msg) {
  /* 旧取帧法 */
  /*n_ret_ = MV_CC_GetImageForBGR(handle_, p_data_, n_data_size_, &st_img_info_,
  1000); struct timeval get_mat_time; gettimeofday(&get_mat_time,NULL); cv::Mat
  frame(st_img_info_.nHeight, st_img_info_.nWidth, CV_8UC3,
...
bool HikvisionBasicCam::SetGamma(float val, unsigned char selector) {
  ...
  if (CurrentValue.nCurValue == PixelType_Gvsp_BayerRG8) {
    ...
    if (selector == MV_GAMMA_SELECTOR_SRGB) {
      n_ret_ =
          MV_CC_SetEnumValue(handle_, "GammaSelector", MV_GAMMA_SELECTOR_SRGB);
      if (MV_OK != n_ret_) {
      ...
      n_ret_ =
          MV_CC_SetEnumValue(handle_, "GammaSelector", MV_GAMMA_SELECTOR_USER);
      if (MV_OK != n_ret_) {
        TDT_ERROR("MV_CC_SetGammaSelector fail! n_ret_ [%x]\n", n_ret_);
        MVCC_ENUMVALUE CurrentValue = {0};
        n_ret_ = MV_CC_GetEnumValue(handle_, "GammaSelector", &CurrentValue);
...
bool HikvisionBasicCam::SetPixelformat(uint32_t pixelformat) {
  n_ret_ = MV_CC_SetEnumValue(
      handle_, "PixelFormat",
      pixelformat);  // 设定像素的类型,TODO:但是没有看懂像素的两个类型是什么
  if (MV_OK != n_ret_) {
...
bool HikvisionBasicCam::SetReverse_X(bool x) {
  // auto A = std::make_shared<MV_CC_FLIP_IMAGE_PARAM>();
  // A->enFlipType = MV_FLIP_HORIZONTAL;
  n_ret_ = MV_CC_SetBoolValue(handle_, "ReverseX", x);
  if (MV_OK != n_ret_) {
...
bool HikvisionBasicCam::SetReverse_Y(bool y) {
  // auto A = MV_CC_FLIP_IMAGE_PARAM();
  // A->enFlipType = MV_FLIP_HORIZONTAL;
  n_ret_ = MV_CC_SetBoolValue(handle_, "ReverseY", y);
  if (MV_OK != n_ret_) {
    TDT_ERROR("MV_CC_FlipImage fail! n_ret_ [%x]\n", n_ret_);
    return false;
  }
