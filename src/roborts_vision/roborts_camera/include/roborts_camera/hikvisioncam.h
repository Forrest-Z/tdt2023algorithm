...
class HikvisionBasicCam {
 public:
  ...
  bool InitHandle(
      std::string cam_guid);  // 设备连接//为什么要两个版本
                              // //这个版本是要打开guid制定的设备，但是
  ...
  bool GetMat_Triger(
      sensor_msgs::msg::Image &image_msg);  // 触发取帧获取Opencv Mat图像
  bool GetMat(
      sensor_msgs::msg::Image &image_msg);  // 非触发取帧获取Opencv Mat图像
  ...
  bool SetReverse_X(bool x = 0);
  bool SetReverse_Y(bool y = 0);
  bool sendTrigger();
