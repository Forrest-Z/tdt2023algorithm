...
namespace robots_perception {
    ...
    bool YoloDetector::parse_yolov5(float *output_blob, float cof_threshold, vector<Rect> &o_rect, vector<float> &o_rect_cof, vector<vector<float>> &points, vector<int> &id) {
    ...
    bool YoloDetector::init(string xml_path, double cof_threshold, double nms_area_threshold, int output_width, int output_height, int use_gpu) {
        ...
        input.tensor().set_layout("NHWC").set_element_type(ov::element::u8).set_shape({1, 480, 640, 3}).set_color_format(ov::preprocess::ColorFormat::BGR);
        // 模型设置
        input.model().set_layout("NCHW");
        // 预处理设置
        uint16_t t[4] = {0, 0, 80, 0};
        input.preprocess().convert_layout("NCHW").convert_color(ov::preprocess::ColorFormat::RGB).convert_element_type(ov::element::f16).scale({255.0f, 255.0f, 255.0f}).custom([=](const ov::Output<ov::Node> &node) { return std::make_shared<ov::opset1::Pad>(node, std::make_shared<ov::opset1::Constant>(ov::element::u16, ov::Shape{4}, t)->output(0), std::make_shared<ov::opset1::Constant>(ov::element::u16, ov::Shape{4}, t)->output(0), std::make_shared<ov::opset1::Constant>(ov::element::f16, ov::Shape{}, float(114.0 / 255.0))->output(0), ov::op::PadMode::CONSTANT); });
        ...
        if (this->gpu_mode == 1 || this->gpu_mode == 2 || this->gpu_mode == 3) {
            ...
            for (auto &&property : gpu_properties) {
                if (property != ov::supported_properties.name()) {
                    if (property.is_mutable()) {
                        if (property == "INFERENCE_PRECISION_HINT") {
                            core.set_property("GPU", ov::hint::inference_precision(ov::element::f32));
                            continue;
                        }
                        if (property == "PERFORMANCE_HINT") {
                        ...
                        if (property == "COMPILATION_NUM_THREADS") {
                            core.set_property("GPU", ov::compilation_num_threads(20));
                            continue;
                        }
                    }
                }
    ...
    //释放资源
    bool YoloDetector::uninit() { return true; }
    ...
    //处理图像获取结果
    bool YoloDetector::process_frame(Mat &inframe, vector<Object> &detected_objects) {
        ...
        vector<vector<float>> origin_point;
        vector<int>           id;

        output_tensor = infer_request.get_output_tensor(0);
        auto out_data = output_tensor.data<float>();

        parse_yolov5(out_data, _cof_threshold, origin_rect, origin_rect_cof, origin_point, id);
        ...
        for (int i = 0; i < final_id.size(); ++i) {
            ...
            detected_objects.push_back(Object{origin_rect_cof[final_id[i]], id[final_id[i]], rawrect, output_points});
        }
    ...
    //以下为工具函数
    double YoloDetector::sigmoid(double x) { return (1 / (1 + exp(-x))); }
} // namespace robots_perception