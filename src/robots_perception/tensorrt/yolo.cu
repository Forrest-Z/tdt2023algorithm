...
using namespace std;
static const int point_num = 4;  //关键点个数
...
const int NUM_BOX_ELEMENT = 15;  // left, top, right, bottom, confidence, class,
...
static __global__ void decode_kernel_v5_face(float *predict, int num_bboxes,
                                             int num_classes, int output_cdim,
                                             float confidence_threshold,
                                             float *invert_affine_matrix,
  ...
  float *class_confidence = pitem + 5 + 2 * point_num;  //四个关键点
  float confidence = *class_confidence++;
  int label = 0;
  for (int i = 1; i < num_classes; ++i, ++class_confidence) {
    if (*class_confidence > confidence) {
...
static __global__ void decode_single_mask_kernel(
    int left, int top, float *mask_weights, float *mask_predict, int mask_width,
    int mask_height, unsigned char *mask_out, int mask_dim, int out_width,
    int out_height) {
 ...
 public:
  ...
  virtual vector<BoxArray> forwards(const vector<Image> &images,
    ...
    std::chrono ::high_resolution_clock::time_point a6 =
        std::chrono::high_resolution_clock::now();
    auto time_used6 =
        std::chrono::duration_cast<std::chrono::duration<double>>(a6 - a5);
    // INFO("decode_single_mask time: %f", time_used6.count() * 1000);
