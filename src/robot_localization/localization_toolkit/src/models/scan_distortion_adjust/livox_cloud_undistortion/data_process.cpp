// #include "../../../../include/tools/condition_judge.hpp"
#include "localization_models/scan_distortion_adjust/livox_cloud_undistortion/data_process.h"
...
{
    ros2_node_ = rclcpp::Node::make_shared("lidar_odom_end_node");
...
void ImuProcess::Reset() 
{
  ...
  std::cout<<"\033[1m\033[33m"<<"Reset ImuProcess"<<"\033[1m\033[33m"<<std::endl;
  b_first_frame_ = true;
  last_lidar_ = nullptr;
  last_imu_ = nullptr;
...
void ImuProcess::IntegrateGyr(const std::vector<sensor_msgs::msg::Imu::ConstPtr> &v_imu) 
{
  ...
  gyr_int_.Reset(last_lidar_->header.stamp.sec + last_lidar_->header.stamp.nanosec* 1e-9, last_imu_);

  /// And then integrate all the imu measurements
  for (const auto &imu : v_imu) {
    gyr_int_.Integrate(imu);
  }
  printf("integrate rotation angle [x, y, z]: [%.2f, %.2f, %.2f]",
...
void ImuProcess::Process(const MeasureGroup &meas) 
{
  ...
  judge::fatal(meas.imu.empty(),"meas.imu.empty()");
  judge::fatal((meas.lidar == nullptr),"meas.imu.empty()");
  ...
  if (b_first_frame_) {
    ...
    std::cout<<"\033[1m\033[33m"<<"The very first lidar frames"<<"\033[1m\033[33m"<<std::endl;
  ...
  dt_l_c_ =
      pcl_in_msg->header.stamp.sec+pcl_in_msg->header.stamp.nanosec* 1e-9 - last_lidar_->header.stamp.sec+last_lidar_->header.stamp.nanosec* 1e-9;
  //// Get input pcl
  ...
  {
    static rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pub_UndistortPcl = 
            ros2_node_->create_publisher<sensor_msgs::msg::PointCloud2>("/livox_first_point", 100);
  ...
  {
    static rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pub_UndistortPcl = 
            ros2_node_->create_publisher<sensor_msgs::msg::PointCloud2>("/livox_undistort", 100);
    // static ros::Publisher pub_UndistortPcl =
  ...
  {
    static rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pub_UndistortPcl = 
            ros2_node_->create_publisher<sensor_msgs::msg::PointCloud2>("/livox_origin", 100);
    // static ros::Publisher pub_UndistortPcl =
