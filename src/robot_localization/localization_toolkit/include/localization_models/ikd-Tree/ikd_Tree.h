...
private:
    ...
    void BuildTree(KD_TREE_NODE **root, int l, int r, PointVector &Storage,
                     bool use_multi_thread = false, int working_thread_num = 0, int worker_thread_max = 0);
...
public:
    ...
    void save_ikd_tree_into_bin(const std::string bin_path);
    void Serialize(KD_TREE_NODE *node, std::ofstream &out) ;
    KD_TREE_NODE* Deserialize(std::ifstream &in);
    void load_ikd_tree_from_bin(const std::string bin_path);
    void BuildAndSave(PointVector point_cloud, const std::string bin_pan_to_save);
    void LoadAndBuild(const std::string bin_pan_to_load);
