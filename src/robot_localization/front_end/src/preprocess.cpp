...
{  
  ...
  *pcl_out = pl_surf;
    ...
    // int size = pcl_out->size();
    // for (int i = 0; i < size; i++)
    // {
    //     compensateLidarMeasurement(&pcl_out->points[i],&pcl_out->points[i],ImuMount_compensate_matrix3d);
    // }
    correctPointCloud(*pcl_out,ImuMount_compensate_matrix3d);
  ...
  //处理离雷达30cm的点云以及z方向上小于 +10cm的点云
    ...
    pass.setFilterLimitsNegative (true);
    pass.setFilterLimits (-100.0,+0.42 );
    pass.filter (*pcl_out);
    //处理离雷达30cm的点云以及z方向上小于 +20cm的点云
    //  std::cout<<"PublishData current_cloud_data_.cloud_ptr size: "<<pcl_out->size()<<"\n";
...
{
  //清除pl_surf、pl_corn和pl_full向量，用于存储平面点、角点和完整点集
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  double t1 = omp_get_wtime();  //使用omp_get_wtime()获取当前时间，记录处理开始时间
  int plsize = msg->point_num;  // 获取msg中点云的大小，存储在plsize中

  //为pl_corn、pl_surf和pl_full向量预分配存储空间
  pl_corn.reserve(plsize);
  pl_surf.reserve(plsize);
  pl_full.resize(plsize);

  // //使用循环初始化pl_buff，这是一个用于存储平面点的二维向量
  // for(int i=0; i<N_SCANS; i++)
  // {
  //   pl_buff[i].clear();
  //   pl_buff[i].reserve(plsize);
  // }
  uint valid_num = 0; //初始化valid_num，用于跟踪有效点的数量
  
  //遍历输入点云的点，检查它们是否属于合法的扫描线和点标签。如果满足条件，则 valid_num++
  for(uint i=1; i<plsize; i++)
  {
    if((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
    {
      ...
      if (valid_num % point_filter_num == 0)
      {
        ...
        {
          //检查当前点与前一个点的位置差异，如果它们的距离足够大且距离LiDAR的距离大于盲区半径，则将当前点添加到pl_surf向量中
...
void Preprocess::oust64_handler(const sensor_msgs::msg::PointCloud2::ConstPtr &msg)
{
  ...
  double time_stamp = time_to_double(msg->header.stamp);
...
void Preprocess::hesai_handler(const sensor_msgs::msg::PointCloud2::ConstPtr &msg)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
...
void Preprocess::pub_func(PointCloudXYZI &pl, const builtin_interfaces::msg::Time &ct)
{
