...
Eigen::Matrix<double, 24, 24> df_dx_input(state_input &s, const input_ikfom &in)
{
	...
	//fix a function in Eigen quaternion
...
/**
 * @brief 点云对齐，主要目标是找到点云观测和ikdtree地图中的匹配点，并构建观测矩阵，以便进行状态估计
 * @param 
 **/
void h_model_output(state_output &s, esekfom::dyn_share_modified<double> &ekfom_data)
{
	...
	// omp_set_num_threads(8);  // Set the number of threads in OpenMP
...
// #pragma omp parallel for
		...
		{	
			...
			else
			{
				...
				if (esti_plane(pabcd, points_near, plane_thr)) //(planeValid)
				{
					...
					// 条件筛选： 如果成功拟合出平面，检查当前点与平面的距离是否满足某些条件 if (p_body.norm() > match_s * pd2 * pd2)
					// 如果满足条件，那么就将该点视为有效的匹配点，用于后续的状态估计
					if (p_body.norm() > match_s * pd2 * pd2)
					{
	...
	// 状态观测矩阵构建：如果至少有一个有效的匹配点，那么它就开始构建状态观测矩阵ekfom_data.h_x。
	// 对于每个选定的匹配点，它计算与平面法向量相关的几个量，并将这些量添加到状态观测矩阵中。
	// 这个状态观测矩阵最后将用于EKF（扩展卡尔曼滤波器）或其他滤波器进行状态估计。
	if (effect_num_k == 0) 
	{
		ekfom_data.valid = false;
		return;
