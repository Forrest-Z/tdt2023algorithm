// #include "livox_dedistortion_pkg/gyr_int.h"
#include "localization_models/scan_distortion_adjust/livox_cloud_undistortion/gyr_int.h"
...
void GyrInt::Integrate(const sensor_msgs::msg::Imu::ConstPtr &imu) {
  ...
  if (v_rot_.empty()) 
  {
    ...
    // {
    //   // RCLCPP_FATAL()
    //   std::cout<<"!!!!!!"<<"start_timestamp_ <= 0 成立"<<"\n";
    //   rclcpp::shutdown();
    //   exit(0);
    // }
    ...
    judge::fatal(start_timestamp_ <= 0,"start_timestamp_ <= 0");
    judge::fatal((last_imu_== nullptr),"last_imu_= nullptr");
    ...
    double dt1 = start_timestamp_ - last_imu_->header.stamp.sec + last_imu_->header.stamp.nanosec* 1e-9;

    // double dt2 = imu->header.stamp.toSec() - start_timestamp_;
    double dt2 = imu->header.stamp.sec+imu->header.stamp.nanosec* 1e-9 - start_timestamp_;

    judge::fatal((dt1 < 0 || dt2 < 0),"dt1 < 0 || dt2 < 0");
    ...
    int32_t trans_sec = (int32_t)start_timestamp_;
    uint32_t trans_nanosec = (start_timestamp_-trans_sec)* 1e+9;    
    ros2_time.sec = trans_sec;ros2_time.nanosec = trans_nanosec;
    imu_inter->header.stamp = ros2_time;
  ...
  const double &time_last = imumsg_last->header.stamp.sec + imumsg_last->header.stamp.nanosec* 1e-9;
  ...
  double time = imu->header.stamp.sec + imu->header.stamp.nanosec* 1e-9;
