...
#include "robot_localization/mapping/lidar_odom_end/lidar_odom_end_flow.hpp"
...
namespace robot_localization
{
    ...
    LidarOdomEndFlow::LidarOdomEndFlow(std::shared_ptr<rclcpp::Node>& node_)
    {
        ...
        global_frame_id = user_config["global_frame_id"+environment].as<std::string>();
        imu_raw_data_topic = user_config["imu"+environment].as<std::string>();
        imu_synced_data_topic = user_config["imu_synced"+environment].as<std::string>();
        imu_link = user_config["imu_link"+environment].as<std::string>();
        ...
        undistorted_pointcloud_topic = user_config["undistorted_pointcloud"+environment].as<std::string>();
        coordinate_transformation = Eigen::Vector3d(
                                                    user_config["init_position"+environment][0].as<double>(),
                                                    user_config["init_position"+environment][1].as<double>(),
                                                    user_config["init_position"+environment][2].as<double>()
                                                   );
        
        if_odom_end_tf_broadcast = user_config["odom_end_send_tf"].as<bool>();
        ...
        // 3.IMU 同步测量
        imu_synced_sub_ptr_ = std::make_shared<ImuSubscriber>(node_, imu_synced_data_topic,100000);
        // 4.lidar to imu tf
        ...
        laser_odom_pub_ptr_ = std::make_shared<OdometryPublisher>(node_, "laser_localization", global_frame_id, "lidar", 100);
        // 5.fused psoe in map frame
        fused_odom_pub_ptr_ = std::make_shared<OdometryPublisher>(node_, "fused_odom", global_frame_id, "lidar", 100);
        // 6.tf
        laser_tf_pub_ptr_ = std::make_shared<TFBroadcaster>(node_, global_frame_id, car_base_link);
        
        key_odom_frame_pub_ptr_ = std::make_shared<KeyFramePublisher>(node_, "/fused_odom_frame","fused_odom",100);
        // 里程计端算法
...
// green_info("to ReadData");
        if(!ReadData())
            return false;
// green_info("ReadData done");

        printf("imu_synced_data_buff_:%d\n",imu_synced_data_buff_.size());
        printf("cloud_data_buff_:%d\n",cloud_data_buff_.size());
    ...
    bool LidarOdomEndFlow::HasData()
    {
        ...
        else
        {
            ...
            {   green_info("to !HasIMUData() && !HasLidarData()()");
                return false;
            }
        }
    ...
    bool LidarOdomEndFlow::PublishFusionOdom()
    {
        ...
        static KeyFrame fused_odom_frame;
        fused_odom_frame.pose = fused_pose_.cast<float>();
        fused_odom_frame.time = current_imu_raw_data_.time_stamp_;
        fused_odom_frame.index++;
        key_odom_frame_pub_ptr_->Publish(fused_odom_frame);
